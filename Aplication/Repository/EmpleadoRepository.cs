...
namespace Aplication.Repository
{
    public class EmpleadoRepository : GenericIntRepository<Empleado>, IEmpleado
    {
        ...
        public EmpleadoRepository(ApiDbContext context)
            : base(context)
        {
        ...
        {
            var EmployeesWithClientsIds = await context
                .Clientes
                .Select(c => c.EmpleadoId)
                .ToListAsync();
            var r = await context
                .Empleados
                .Where(c => !EmployeesWithClientsIds.Contains(c.Id))
                .Include(c => c.Oficina)
                .ToListAsync();
            return r;
        }
        ...
        public async Task<IEnumerable<Empleado>> EmployeesWithoutOfficeNorCustomers()
        {
            ...
            var r = await context
                .Empleados
                .Where(c => !EmployeesWithClientsIds.Contains(c.Id) && c.Oficina == null)
                .ToListAsync();
            return r;
        }
        ...
        public async Task<IEnumerable<Empleado>> GetEmployeeWithBossWithBoss()
        {
            var r = await context
                .Empleados
                .Include(m => m.Jefe)
                .ThenInclude(m => m.Jefe)
                .ToListAsync();
            return r;
        }

        public async Task<IEnumerable<Empleado>> SalesRepresentativesEmployees()
        {
            var r = await context
                .Empleados
                .Where(p => p.Puesto.ToLower() == "representante ventas")
